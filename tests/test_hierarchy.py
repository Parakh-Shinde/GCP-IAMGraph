...
def test_effective_bindings_include_ancestors():
    resources = [
        Resource.from_dict({"name": "organizations/1", "type": "organization", "bindings": [{"role": "roles/viewer", "members": ["user:a@test"]}]}),
        Resource.from_dict({"name": "projects/p", "type": "project", "parent": "organizations/1"}),
    ]
